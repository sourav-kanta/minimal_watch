...
LOG_MODULE_REGISTER(WEATHER_APP, LOG_LEVEL_INF);
...
static void refresh_weather_ui_data(void) {
    if (!current_temp_lbl || !humidity_lbl || !precip_lbl || !wind_lbl || !main_icon) {
        LOG_WRN("Weather UI elements not active. Data cached cleanly in background.");
        return;
    }
    ...
    LOG_INF("Refreshing weather UI elements with fresh BLE payload data...");
...
static void container_key_event_cb(lv_event_t * e) {
    ...
    if (code == LV_EVENT_KEY) {
        ...
        if (key == LV_KEY_ENTER) {
            LOG_INF("Container Enter pressed -> Focusing single_card");
...
static void receive_event(app_update_t* update) {
    switch(update->req) {
        case DATED_WEATHER_REQUEST :
            LOG_INF("Received fresh 24-hour block via BLE channel.");
            memcpy(hourly_forecast, update->data, sizeof(hourly_weather_t)*TOTAL_HOURS);
            date_active_held = false;
            refresh_weather_ui_data();
            break;
        default:
            LOG_WRN("Unhandled weather app update opcode: %d", update->req);
            break;
    }
}
