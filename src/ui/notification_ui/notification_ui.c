...
LOG_MODULE_REGISTER(notification_ui, LOG_LEVEL_INF);
...
static void auto_scroll_to_card(lv_obj_t* card) {
    ...
    if (lv_obj_get_height(card) == 0) {
        ...
        if (cd && cd->idx < VISIBLE_CARDS) {
            ...
            LOG_INF("Layout dirty! Using precalculated backup scroll Y: %d for index %d", 
                    backup_scroll_y, cd->idx);
            lv_obj_scroll_to_y(scroll_container, backup_scroll_y, true);
        } else {
            LOG_ERR("Layout dirty and card metadata missing or out of bounds");
        }
        return;
    }
    lv_area_t card_area;
    ...
    LOG_INF("Layout stable. Auto scroll check - Top: %d, Bottom: %d", card_top, card_bottom);
...
static void title_key_cb(lv_event_t *e) {
    ...
    if (code == LV_EVENT_KEY) {
        ...
        if (key == LV_KEY_ENTER) {
            ...
            lv_obj_t* parent = find_first_focusable_parent_dfs(lbl_title);
            if(!parent) return;
...
static void dismiss_handler_cb(lv_event_t* e) {
    ...
    if(noti_curr_base_idx + VISIBLE_CARDS >= noti_snapshot_len) {
        noti_curr_base_idx--;
        noti_curr_base_idx = noti_curr_base_idx < 0 ? 0 : noti_curr_base_idx;
    }
...
static void populate_cards(lv_obj_t* parent, const notification_t* notif_list,
    ...
    for (unsigned int i = noti_curr_base_idx; i < display_card_len; i++) {
        ...
        lv_obj_set_style_text_color(lbl_title, COLOR_THEME_TEXT_PRIMARY, LV_PART_MAIN | LV_STATE_DEFAULT);
...
static void rebase_and_populate_cards(unsigned int idx) {
    LOG_INF("Rebasing and repopulating");
    if(idx == 0) {
...
static void handle_notification_screen_keys_cb(lv_event_t * e) {
    LOG_INF("Received callback on parent");
    lv_event_code_t code = lv_event_get_code(e);
    if(code == LV_EVENT_KEY) {
        uint32_t key = lv_event_get_key(e); 
        lv_obj_t* curr_focused = lv_group_get_focused(get_current_group());
        ...
        if(key == LV_KEY_ESC) {
            ...
            if(curr_focused == notification_scr) {
                lv_event_stop_bubbling(e);
                return;
            }
            else {
        ...
        if(key == LV_KEY_ENTER) {
            if(!curr_focused) return;
            lv_obj_t* child = find_first_focusable_child_dfs(curr_focused);
            if(child)
...
void draw_notification_ui(lv_obj_t* root) {
    if (!root) return;
    LOG_INF("Inside notifications setup");
    ...
    make_obj_navigable(notification_scr);
    lv_group_focus_obj(notification_scr);
    lv_obj_add_flag(notification_scr, LV_OBJ_FLAG_EVENT_BUBBLE);
    lv_obj_set_size(notification_scr, lv_pct(100), lv_pct(100));
    
    lv_obj_add_event_cb(notification_scr, handle_notification_screen_keys_cb, LV_EVENT_KEY, NULL);
    
    lv_obj_move_foreground(notification_scr);
    ...
    notif_list = get_all_notifications(&noti_snapshot_len);
...
void del_notification_ui(lv_obj_t* root) {
